...
| [`software/`](https://gitlab.com/yguang1/software/-/tree/feat/ddr_mem) | Submodule: QubiC host software plus the Ant-Q PS servers (`scripts/batch_server.c`, `scripts/dma_server.c`) and the batch client (`qubic/rpc_client.py`). `d04d09b` is the C3 stage as one squashed commit on the fork's `feat/ddr_mem`, the product subset of the last measured commit `a1eefb5` (tag `antq-qst-software-a1eefb5`): same product code plus one guard so that the default assembly path also works with an unpatched `distributed_processor`, minus the measurement-only files, which now live in `experiments/board/` (`std_server.c` + `std_client.py`, the stock PS path in C used as the timing baseline, and the two board deployment scripts) | `d04d09b` (fork `feat/ddr_mem`) |
