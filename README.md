...
| [`software/`](https://gitlab.com/yguang1/software/-/tree/feat/ddr_mem) | Submodule: QubiC host software plus the Ant-Q PS servers (`scripts/batch_server.c`, `scripts/dma_server.c`, `scripts/std_server.c`) and the batch client (`qubic/rpc_client.py`). `29e7fcb` is the C3 stage as one squashed commit on the fork's `feat/ddr_mem`; its tree equals the last measured commit `a1eefb5` (tag `antq-qst-software-a1eefb5`) plus one guard so that the default assembly path also works with an unpatched `distributed_processor` | `29e7fcb` (fork `feat/ddr_mem`) |
