...
| [`software/`](https://gitlab.com/yguang1/software/-/tree/ddr-stop-c3) | Submodule: QubiC host software plus the Ant-Q PS servers (`scripts/batch_server.c`, `scripts/dma_server.c`, `scripts/std_server.c`) and the batch client (`qubic/rpc_client.py`). `36be123` = the measured `a1eefb5` plus one guard so that the default assembly path also works with an unpatched `distributed_processor` | `36be123` (branch `ddr-stop-c3`) |
...
| Software | `software_commit` column of every row; the final rows use `a1eefb5` and its predecessors on `ddr-stop-c3` (the commit of each fix is named in the reports) | the pinned `36be123` adds only the optional-argument guard in `qubic/toolchain.py` |
...
| Downlink (command streaming, ping-pong command buffers, `circuit_not_ready` detector) | `gateware/`, `software/` | Integrated with the uplink in the C3 image, board-validated, measured. The official QubiC `feat/ddr_mem` branch carries an earlier prototype; the integrated version is under review upstream ([gateware MR !38](https://gitlab.com/LBL-QubiC/gateware/-/merge_requests/38), [software MR !47](https://gitlab.com/LBL-QubiC/software/-/merge_requests/47)), so the submodules point at the authors' fork until they are merged |
| Pool-wide envelope/frequency tables (heterogeneous batches without table reloads) | `software/` + `experiments/patches/` | Implemented in the host toolchain (two-pass assembly); the `distributed_processor` extension (patch against `c22cce8`, one optional argument) is being submitted upstream; the software's default assembly path does not need it |
