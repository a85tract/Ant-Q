...
be busy; "realized" QPU time uses the shot length measured on the board (the slope method, `per_shot_realized_real.csv`),
which is 0.14-0.17 us longer per shot than the compiled value for every program.
...
| RQ3, Table "noise study" | Readability of the 20 circuits on six Qiskit fake backends over 20 seeds each: the six circuits that fail on at least one backend, the stop checkpoint distribution, and 0 false stops in 1698 readable runs / 0 missed stops in 117 unreadable runs | `run_rq3_stats.sh` (`CIRCUITS=v2 REF=exact`), `make_rq3_tables.py`, `rq3_adequacy_gate.py` | `results/rq3_v2/rq3_<backend>.csv` (one row per circuit x seed), `results/table_rq3_cells_v2.csv` (per cell), `results/rq3_v2/adequacy_gate.csv`, addendum sections E-F |
