...
// [stop] early-stop support: actualshots = completed shots latched at DONE; stopped = stopreq seen at DONE
reg [31:0] actualshots=0;
reg stopped=0;
...
assign dspif.actualshots=actualshots;
assign dspif.stopped=stopped;
...
always @(*) begin
	...
	case (state)
		...
		MORESHOT: begin
			...
			nextstate=(shotadd && !dspif.stopreq) ? SHOTADD : DONE; // [stop] no new shot once stopreq is visible
		end
		SHOTADD: begin
			nextstate=START;
		end
	...
	if (dspif.reset) begin
		...
		case (nextstate)
			...
			DONE: begin
				...
				actualshots<=shotcnt+32'd1; // [stop] pre-edge shotcnt = index of the shot just completed
				stopped<=dspif.stopreq;
				shotcnt<=0;
				procreset<=1'b1;
			end
		endcase
