...
| `run_phys_campaign.py` | The six physics experiments on c3 / c1 / std, including the calibration series used to fit the realized loop period. Experiments (5) and (6) run as sub-circuit streams and **must be run with `--pool-tables`** (as `run_after_A2.sh` does): a drive-only segment carries empty readout tables, which the client's table-grouping predicate treats as a conflict with the final segment's tables; the pool gives every segment identical tables so the stream runs as one hardware group. On images with fewer cores than a circuit needs, the pool is built from the circuits that fit. |
...
| `std_server.c`, `std_client.py` | The stock QubiC PS path (BRAM command load + accumulator readback) re-implemented in C, with its host client: the `std` baseline of every timing table, so that the baseline pays no Python interpreter cost per MMIO word. Measurement instruments, not QubiC features; deliberately kept out of the software merge request. |
| `deploy_servers.sh`, `deploy_c3_veneno.sh`, `deploy_slot_veneno.sh` | Push the PS servers (`dma_server.c`, `batch_server.c` from the software submodule, `std_server.c` from here) and a gateware build's bits slot to the authors' board and restart the QubiC service. Host names are the authors'. |
