...
| `std_server.c`, `std_client.py`, `test_std_client.py` | The stock QubiC PS path (BRAM command load + accumulator readback) re-implemented in C, with its host client: the `std` baseline of every timing table, so that the baseline pays no Python interpreter cost per MMIO word. `test_std_client.py` checks on the host that the client's write list equals, word for word, the sequence the Python path performs. Measurement instruments, not QubiC features; deliberately kept out of the software merge request. |
